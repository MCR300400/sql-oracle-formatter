  ...
  <div>
    ...
    <div v-if="localUseLeftJoin">
      <div v-for="(join, joinIndex) in localJoins" :key="joinIndex">
        ...
        <div v-if="join.availableJoinColumns.length" @change="updateQuery">
          <label for="joinColumns">Colonne della Tabella JOIN:</label>
          <input
              type="checkbox"
              @change="toggleSelectAllJoinSub(joinIndex)"
              :checked="areAllSelectedJoin(joinIndex)"
          />
          <label>Seleziona Tutte</label>
          <div class="checkbox-container">
            <div v-for="column in join.availableJoinColumns" :key="column" class="checkbox-item">
              <input
                  type="checkbox"
                  :value="column"
                  v-model="join.selectedJoinColumns"
                  :id="'join_' + column"
                  @change="emitJoins"
              />
