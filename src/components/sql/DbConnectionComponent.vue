...
<script>
import { loadColumns, loadColumnsDefault, loadColumnsExtra, loadColumnsKey, loadColumnsNull, loadColumnsTipo } from '../../stores/ColonneStore.js';
...
export default {
  ...
  methods: {
    ...
    connectToDb() {
      ...
      localStorage.setItem('fusoorario', this.fusoorario);
      localStorage.setItem('tableName', this.tableName);
      localStorage.setItem('usernameDB', this.usernameDB);
      localStorage.setItem('passwordDB', this.passwordDB);
      ...
      loadColumns(connectionDetails);
      loadColumnsTipo(connectionDetails);
      loadColumnsNull(connectionDetails);
      loadColumnsKey(connectionDetails);
      loadColumnsDefault(connectionDetails);
      loadColumnsExtra(connectionDetails)
      .then(columns => {
          if (columns) {
  ...
  mounted() {
    ...
    this.fusoorario = localStorage.getItem('fusoorario') || '';
    this.tableName = localStorage.getItem('tableName') || '';
    this.usernameDB = localStorage.getItem('usernameDB') || '';
    this.passwordDB = localStorage.getItem('passwordDB') || '';
  }
};
</script>
